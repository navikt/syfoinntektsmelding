...
class DatapakkePublisherJob(
    ...
    override fun doJob() {
        ...
        val populatedDatapakke = datapakkeTemplate
            ...
            .replace("@FF_bucket1", forsinkelseWeeklyStats.filter { it.bucket == 1 }.map { it.antall_med_forsinkelsen_lps+it.antall_med_forsinkelsen_altinn }.joinToString())
            .replace("@FF_bucket2", forsinkelseWeeklyStats.filter { it.bucket == 2 }.map { it.antall_med_forsinkelsen_lps+it.antall_med_forsinkelsen_altinn }.joinToString())
            .replace("@FF_bucket3", forsinkelseWeeklyStats.filter { it.bucket == 3 }.map { it.antall_med_forsinkelsen_lps+it.antall_med_forsinkelsen_altinn }.joinToString())
            .replace("@FF_bucket4", forsinkelseWeeklyStats.filter { it.bucket == 4 }.map { it.antall_med_forsinkelsen_lps+it.antall_med_forsinkelsen_altinn }.joinToString())
