...
fun getRandomDate(before: LocalDateTime?): String {
    return if (before == null)
        "${(2000..LocalDate.now().year).shuffled().first()}-${
            addLeadingZeroIfLessThanTen(
                (1..12).shuffled().first()
            )
        }-${addLeadingZeroIfLessThanTen((1..28).shuffled().first())}"
    else
        "${(2000 until before.year).shuffled().first()}-${
            addLeadingZeroIfLessThanTen(
                (1 until before.monthValue).shuffled().first()
            )
        }-${addLeadingZeroIfLessThanTen((1 until before.dayOfMonth).shuffled().first())}"
}

fun getRandomTime(before: LocalDateTime?): String {
    return if (before == null)
        "${addLeadingZeroIfLessThanTen((0..24).shuffled().first())}:${
            addLeadingZeroIfLessThanTen(
                (0..60).shuffled().first()
            )
        }:${addLeadingZeroIfLessThanTen((0..60).shuffled().first())}"
    else
        "${addLeadingZeroIfLessThanTen((0 until before.hour).shuffled().first())}:${
            addLeadingZeroIfLessThanTen(
                (0 until before.minute).shuffled().first()
            )
        }:${addLeadingZeroIfLessThanTen((0 until before.second).shuffled().first())}"
}
