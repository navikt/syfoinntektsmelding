    ...
    override fun getWeeklyStats(): List<IMWeeklyStats> {
        val query = """
            SELECT
                ...
                count(*) filter (where (data -> 'refusjon' ->> 'beloepPrMnd')::numeric > 0 and (data -> 'refusjon' ->> 'beloepPrMnd')::numeric = (data ->> 'beregnetInntekt')::numeric and data ->> 'begrunnelseRedusert' != 'IkkeFravaer') as full_refusjon, -- 03A
                count(*) filter (where (data -> 'refusjon' ->> 'beloepPrMnd')::numeric > 0 and (data -> 'refusjon' ->> 'beloepPrMnd')::numeric < (data ->> 'beregnetInntekt')::numeric and data ->> 'begrunnelseRedusert' != 'IkkeFravaer') as delvis_refusjon, -- 03A
                count(*) filter (where ((data -> 'refusjon' ->> 'beloepPrMnd') is null or (data -> 'refusjon' ->> 'beloepPrMnd')::numeric = 0) and data ->> 'begrunnelseRedusert' != 'IkkeFravaer') as ingen_refusjon -- 03C
